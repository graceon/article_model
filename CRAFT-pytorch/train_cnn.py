...
def train_epoch(model, loader, optimizer, epoch, n_epochs, print_freq=300):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        # print(torch.ne(pred.squeeze(), target.cpu()).float().sum().item())
...
def train(model, train_set, valid_set, test_set, save, n_epochs=300,
          batch_size=64, lr=0.1, wd=0.0001, momentum=0.9, seed=None,epoch_start=0):
    if seed is not None:
    ...
    for epoch in range(epoch_start,epoch_start+n_epochs):
        ...
        torch.save(model.state_dict(), os.path.join(save, str(epoch+1)+'model.pth'))
        scheduler.step()
...
def demo( depth=100, growth_rate=12, efficient=True, valid_size=5000,
    ...
    print(epoch_start)
    if(epoch_start>0):
        model.load_state_dict(torch.load("cnn_model/"+str(epoch_start)+"model.pth"))
    print(model)
    ...
    # Train the model
    train_set=custom_dataset.custom_dataset("package.pth")
    valid_set=0
    test_set=0
    train(model=model, train_set=train_set, valid_set=valid_set, test_set=test_set, save=save,
          n_epochs=n_epochs, batch_size=batch_size, seed=seed,epoch_start=epoch_start)
    print('Done!')
