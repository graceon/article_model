...
def append_dir_kaggle(package_x,package_y,basepath,item_y,item_y_base):
    ...
    for item in path:
        ...
        item_x*=255
        write_cnt+=1
        dir_path='D:\\refine\\kaggle\\'+str(item_y)+'\\'
        ...
        cv2.imwrite('D:\\refine\\kaggle\\'+str(item_y)+'\\'+"%d.png"%write_cnt,item_x.numpy().astype(int))
...
if __name__ =="__main__":
    ...
    def append_mnist(loader):
        ...
        for item_x,item_y in loader:
            ...
            item_x/=item_x.max()
            item_x*=255
            # print(str(item_y[0].item()))
            # print('D:\\refine\\mnist\\'+item_y[0].item()+'\\')
            write_cnt+=1
            dir_path='D:\\refine\\mnist\\'+str(item_y[0].item())+'\\'
            isExists=os.path.exists(dir_path)
            # 判断结果
            if not isExists:
                # 如果不存在则创建目录
                # 创建目录操作函数
                os.makedirs(dir_path) 
            cv2.imwrite('D:\\refine\\mnist\\'+str(item_y[0].item())+'\\'+"%d.png"%write_cnt,item_x.numpy().astype(int))
            # package_x.append(torch.tensor(item_x,dtype=torch.uint8))
